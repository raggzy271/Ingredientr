...
import random
import numpy as np
from .modules import encoder, transformer_decoder
...
def get_model(args, ingr_vocab_size, instrs_vocab_size):
    ...
    encoder_ingrs = encoder.EncoderLabels(args.embed_size, ingr_vocab_size,
                                  args.dropout_encoder, scale_grad=False).to(device)
    # build image model
    encoder_image = encoder.EncoderCNN(args.embed_size, args.dropout_encoder, args.image_model)

    decoder = transformer_decoder.DecoderTransformer(args.embed_size, instrs_vocab_size,
    ...
    ingr_decoder = transformer_decoder.DecoderTransformer(args.embed_size, ingr_vocab_size, dropout=args.dropout_decoder_i,
                                      ...
                                      scale_embed_grad=False)
    # recipe loss
    criterion = metrics.MaskedCrossEntropyCriterion(ignore_index=[instrs_vocab_size-1], reduce=False)
...
class InverseCookingModel(nn.Module):
    ...
    def forward(self, img_inputs, captions, target_ingrs,
        ...
        if not self.recipe_only:
            ...
            losses['iou'] = utils.metrics.softIoU(pred_one_hot, target_one_hot)
